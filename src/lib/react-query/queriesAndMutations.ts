import type { INewPost, INewUser, IUpdatePost, IUser } from "@/types";
import {
  useQuery,
  useMutation,
  useQueryClient,
...
export const useUpdatePost = () => {
  ...
  return useMutation<IUpdatePost, Error, IUpdatePost>({
    mutationFn: (post: IUpdatePost) => updatePost(post) as Promise<IUpdatePost>,
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_POST_BY_ID, data?.$id],
      });
...
export const useGetUsers = (currentUserId: string) => {
  return useQuery({
    queryKey: [QUERY_KEYS.GET_USERS],
    queryFn: () => getUsers(currentUserId),
  });
};
...
export const useUpdateUser = () => {
  ...
  return useMutation<IUser, Error, IUser>({
    mutationFn: (user: IUser) => updateUser(user) as Promise<IUser>,
    onSuccess: (data) => {
      queryClient.invalidateQueries({
...
export const useFollowUser = () => {
  const queryClient = useQueryClient();
  return useMutation<boolean, Error, FollowVariables>({
    mutationFn: ({ followedUser, followerUser }) =>
      followUser(followedUser, followerUser),
    onSuccess: (_data, variable) => {
      const { followerUser, followedUser } = variable;
      ...
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_FOLLOWERS_LIST, followerUser?.$id],
      });
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_FOLLOWING_LIST, followerUser?.$id],
      });
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_USER_BY_ID, followedUser?.$id],
      });
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_USER_BY_ID, followerUser?.$id],
      });
    },
  });
};
